...
func TestDecodeClientCommandAcceptsCanonicalCommands(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "add CPU player", message: `{"version":1,"direction":"client_command","type":"ADD_CPU_PLAYER","command_id":"cmd-cpu-add","room_id":"room-1","payload":{"team_id":"B"}}`, wantType: CommandAddCPUPlayer, wantPayload: "protocol.AddCPUPlayerPayload"},
		{name: "remove CPU player", message: `{"version":1,"direction":"client_command","type":"REMOVE_CPU_PLAYER","command_id":"cmd-cpu-remove","room_id":"room-1","payload":{"player_id":"cpu-1"}}`, wantType: CommandRemoveCPUPlayer, wantPayload: "protocol.RemoveCPUPlayerPayload"},
		{name: "leave room", message: `{"version":1,"direction":"client_command","type":"LEAVE_ROOM","command_id":"cmd-leave","room_id":"room-1","payload":{}}`, wantType: CommandLeaveRoom, wantPayload: "protocol.EmptyPayload"},
		{name: "kick player", message: `{"version":1,"direction":"client_command","type":"KICK_PLAYER","command_id":"cmd-kick","room_id":"room-1","payload":{"player_id":"player-2"}}`, wantType: CommandKickPlayer, wantPayload: "protocol.KickPlayerPayload"},
		...
		{name: "select move", message: `{"version":1,"direction":"client_command","type":"SELECT_MOVE","command_id":"cmd-5","room_id":"room-1","match_id":"match-1","payload":{"token_id":"token-1","piece_id":"piece-1"}}`, wantType: CommandSelectMove, wantPayload: "protocol.SelectMovePayload", wantMatchID: "match-1"},
