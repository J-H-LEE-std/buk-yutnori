...
전송한다. 서버 snapshot의 후보에는 각 경로의 `previews`를 제공한다. 각 항목은
`route`(백도는 null), `traversed`(서버 계산 통과 칸 배열), `destination_state`,
`destination_space_id`(완주는 null)다. 클라이언트는 말 선택 후 이 배열을 그대로
판 위에 표시한다. 오래된 이벤트에 preview가 없으면 경로를 추론하지 않는다.
이동 확정은 기존 SELECT_MOVE와 필요한 SELECT_ROUTE를 따르며 preview는 판정을
대신하지 않는다. 관전자, 상대 턴, CPU 제어, 재동기화 중과 응답 대기 중에는 모든 게임 입력을
잠근다.
...
네 단계는 한 시점에 하나의 주 화면만 노출하는 독립 view다. 로그인은 중앙 타이틀과
Google 로그인, 메인 로비는 왼쪽 내 정보와 오른쪽 방 목록·하단 채팅, 방 로비는
왼쪽 참가자·관전자와 준비·퇴장 및 오른쪽 플레이어 슬롯·채팅, 게임은 왼쪽 판·채팅과
오른쪽 참가자·우하단 던지기로 배치한다. 긴 목록과 텍스트는 정해진 패널 안에서
스크롤한다. 단일 HTML을 사용하더라도 화면 전환과 정리 책임을 명시적으로 분리한다.

게임 화면은 서버 `YUT_RESULT`의 던진 사람과 한글 결과명을 리소스와 함께 유지
표시한다. 남은 결과 큐와 최근 던지기는 별도 표시하여 CPU가 즉시 소비한 결과도
확인할 수 있다. 재접속 시 남은 큐는 snapshot으로 복원하며 과거 던지기를 추측하지
않는다. 현재 플레이어·팀·입력 단계·제한 시간을 한글로 표시하고 opaque 토큰 ID를
선택 버튼 이름으로 사용하지 않는다. 클라이언트 표시가 서버 타이머를 연장하지 않는다.
...
2. 메인 로비 — 방 목록(`GET /api/v1/rooms`)과 전체 채팅을 표시한다. 방 목록은
   ...
   성공하면 서버가 반환한 방 상세의 멤버 nickname·역할·팀·준비·시작 상태를 표시한다.
   방 상세에 진입한 뒤에는 인증된 WebSocket으로 ROOM_UPDATED를 수신해
   상세를 다시 조회하고, 플레이어의 팀 선택·준비 및 방장의 게임 시작 요청을 서버에
   전달한다. `START_GAME`이 수락되면 서버가 시작 조건과 확인을 최종 판정하고, 클라이언트는
   별도의 사용자 확인 단계를 요구하지 않는다. 명령 결과가 거부되면 서버 오류를 표시하며
   클라이언트는 상태를 추론하지 않는다.
   ...
   브라우저는 이를 `textContent`로 렌더링한다. 인증 후 상단의 `내 프로필`에서
   `GET`/`PUT /api/v1/profile/me`로 닉네임과 전적 공개 여부를 설정하고 서버가 확정한
   승·패를 표시한다. 최초 profile이 없다는 `profile_not_found` 응답은 설정 폼으로
   전환하며, 그 밖의 오류를 빈 profile로 오인하지 않는다.
3. 방 로비 — 입장 성공 후 도달한다. 방 상세 조회(멤버 전용), ROOM_UPDATED
   구독과 팀·준비·게임 시작 명령이 데이터 소스다. 전체 채팅은 계속 `lobby` scope를
...
방 로비의 인간 멤버와 전체 채팅 발신자의 `프로필 보기`는
`GET /api/v1/profiles/{user_id}`만 데이터 소스로 사용한다. 응답에 서버 확정 `wins`와
`losses`가 있으면 승·패를 표시하고, 비공개 profile처럼 두 필드가 없으면 닉네임과
`전적 비공개`만 표시한다. 클라이언트는 누락된 전적을 0으로 추론하지 않는다. CPU는
계정 profile 대상이 아니므로 조회 동작을 제공하지 않는다. nickname을 포함한 모든
profile 응답은 `textContent` 또는 form value로만 반영하고, 저장 중에는 중복 제출을
막는다.
