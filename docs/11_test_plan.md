...
- 결과 큐 32개 상한과 33번째 던지기 거부
- 상한 도달 시 봉인이 해당 턴 종료까지 유지되고 큐가 줄어도 추가 던지기 재개 안 함
- 상한 도달 후 잡기 및 북 잡기의 추가 던지기 억제
- 32번째 결과가 모 또는 북일 때 큐 봉인과 북 우선 처리
- FIFO/free 이동 순서에서 상한 도달 뒤 큐 처리

- 도·개·걸·윷·모
- 연속 윷·모
- 윷 후 북: `[윷, 북]`
- 북 즉시 우선 처리와 북 처리 후 잔여 결과 재개
- FIFO/free 공통의 북 우선 처리
- 사용할 수 없는 토큰만 소멸
- 잡기 즉시 추가 던지기
...
- 인간 1명과 독립 CPU 3명의 균형 팀에서 CPU 선공 뒤 서로 다른 CPU가 연속으로
  완전한 턴을 수행하고 인간 제어로 넘어감
...
- 시작 전 연결 끊김
- 게임 중 단일 이탈
- 다중 탭 중간 연결 종료는 이탈로 처리하지 않고 마지막 연결 종료만 이탈 처리
- 비현재 턴 이탈 뒤 해당 플레이어 턴에 disconnected CPU 대체
- 비현재 플레이어 복귀 시 이탈한 현재 플레이어의 CPU 턴 완료 및 watchdog 취소
- 한 팀 전원 이탈
- 양 팀 전원 이탈 후 29초 복귀
- 양 팀 전원 이탈 30초 무효·폐쇄
- 관전자는 player presence 이벤트와 전원 이탈 판정에서 제외
- 1인+CPU 방도 유일한 인간 이탈 시 CPU 진행을 멈추고 30초 감시
- presence 이벤트 저장 실패로 저장 장애 정지에 들어가도 인증 WebSocket 연결 유지
- 중복 명령
...
- 결과 토큰은 한 번만 소비됨
- 북이 있는 동안 일반 결과 선택을 시작할 수 없음
- 북 토큰 생성 직후 자동 처리되어 미해결 북이 두 개 이상 큐에 공존하지 않는지 검증
