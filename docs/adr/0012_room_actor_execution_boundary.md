...
- 살아 있는 방마다 application 계층의 actor goroutine 하나를 둔다. actor는 해당
  방의 authoritative client command와 서버 소유 내부 상태 전이 실행 순서를
  단독으로 소유한다.
- WebSocket과 향후 HTTP room router는 상태를 직접 변경하지 않고 해당 방 actor에
  명령을 제출한다. 서로 다른 방은 서로 다른 actor에서 독립적으로 실행한다.
- 서버 소유 타이머 만료와 같은 내부 상태 전이는 발생시킨 goroutine에서 방 상태를
  직접 변경하지 않고 같은 actor mailbox에 내부 operation으로 제출한다.
- actor mailbox는 버퍼를 두지 않는다. actor가 현재 요청을 실행 중이면 다음 호출자는
  자신의 context로 admission을 기다리며, 서버 메모리에 무제한 backlog를 만들지
  않는다.
- 호출자 context는 actor가 client command 또는 내부 operation을 수락하기 전까지만
  admission 취소에 사용한다. actor가 수락한 뒤에는 `context.WithoutCancel`로 전달
  값을 보존하되 제출자 취소와 deadline을 분리하고, 해당 실행을 끝까지 완료한다.
  client command는 멱등 결과 확정까지 완료하며, 저장소나 외부 작업의 제한 시간은
  room application이 별도 내부 context로 정한다.
...
- 수락된 client command 또는 내부 operation 실행이 끝나면 actor를 종료하고
  room-lifetime cleanup을 정확히 한 번
...
- actor가 호출한 executor, 내부 operation 또는 cleanup이 panic하면 actor goroutine
  경계에서 이를 복구하고 `ErrRoomActorPanicked`로 기록한다. 해당 방은 즉시 폐쇄하고
  cleanup을 한 번 시도하며, panic 뒤에는 부분 적용 가능성이 있는 방에서 다음 요청을
  실행하지 않는다. executor와 내부 operation panic은 수락된 제출과 `Close` 양쪽에
  terminal error로 전달하고, cleanup panic은 `Close`에 전달한다. 한 방의 panic이
  서버 프로세스나 다른 방 actor를 종료시키지 않는다.
- 내부 operation이 보통 error를 반환하면 제출자에게 그대로 전달하되 방을 자동으로
  폐쇄하지 않는다. operation은 도메인 검증 실패처럼 정상적으로 거부 가능한 전이도
  표현하기 때문이다.
...
- 같은 방의 수락된 client command와 내부 operation은 동시에 실행되지 않고 actor
  admission 순서로 처리된다.
- transport나 내부 제출자가 취소되어도 이미 수락된 실행은 중간 취소되지 않는다.
