# ADR-0015: 레지스트리 방 이벤트 방송과 시작 확인 알림 (superseded)

- 상태: 부분 폐기 — ROOM_UPDATED 방송만 유지; 시작 확인 계약은 즉시 시작으로 대체
- 결정일: 2026-08-23

> 2026-09-10 결정: `START_GAME`은 서버가 즉시 경기 런타임을 확정한다. 신규
> 클라이언트는 `GAME_STARTING`과 `CONFIRM_GAME_START`를 생성하지 않는다. 아래의
> 시작 확인 내용은 과거 이벤트·저장 데이터 호환을 위한 기록으로만 보존한다.
