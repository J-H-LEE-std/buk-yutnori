...
- 백도 후보 부재는 백도 토큰만 소멸시키고 기존 큐 흐름을 따른다.
- 북은 큐에 생성되는 즉시 최우선 처리하며, 최소 완주 거리 동률인 모든 위치 그룹을 이동한다.
- 판 위 북 후보가 없으면 완주하지 않은 대기 말 하나를 서버 난수로 북 위치에 보낸다.
- 강제 지름길의 뒷모 경로가 `방`을 통과하면 `방수기` 방향을 유지한다.
- 위 변경은 `docs/02_game_rules.md`, `docs/03_turn_queue_state_machine.md`,
  `docs/04_board_graph_and_movement.md`, `spec/turn_state_machine.yaml` 및
  Issue #163의 규칙 테스트를 정본으로 한다.

## 구현 차단 항목

- 한 턴 미해결 결과 큐 최대 32개와 상한 도달 시 모든 추가 던지기 봉인은 #183에서
  게임 규칙으로 확정했다. 결과 상한은 `docs/02_game_rules.md`,
  `docs/03_turn_queue_state_machine.md`, `spec/turn_state_machine.yaml`이 정본이다.
...
- #161: 실제 브라우저 수동 테스트에서 네 화면 흐름, 한글 윷 결과, 말 직접 선택,
  종료 후 대기실 복귀를 확인했다. 따라서 Milestone 5의 내부 알파 테스트 진입
  차단은 해소되었다. 다만 최종 아트워크·브라우저 호환성·운영 배포 검증은
  Milestone 6에서 계속한다.
- #161은 snapshot 후보의 `previews`에 기존 서버 이동 계획의 통과 칸·도착점을
  투영한다. 저장된 MOVE_REQUIRED 이벤트 계약은 유지하며, preview가 없는 과거
  이벤트에서는 궤적을 추론하지 않는다. 클라이언트는 렌더 좌표만 C에서 조회한다.
