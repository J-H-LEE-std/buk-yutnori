...
- 시작 확인 창과 대기실 명령의 상호작용 제한. `SET_READY`가 레지스트리에 연결된
  상태에서 시작 확인(ADR-0003)을 흐름에 붙일 때, 확인 진행 중 준비 해제·팀 변경이
  확인 로직이 참조하는 준비 상태를 바꾸지 않도록 차단하거나 설계에 반영해야 한다.
  시작 요청 흐름을 레지스트리에 연결하는 후속 작업 착수 전에 먼저 되짚는다.
