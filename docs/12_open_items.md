# 남은 항목

## 구현 차단 항목

이번 명세 정합성 수정으로 전체 보드, 북 후보, 완주 거리, 백도 경로 상태의 알려진 구현 차단 항목은 해소되었다. 전체 보드 그래프는 `spec/board_graph.yaml`이 정본이며 추가 보드 자료를 기다리지 않는다.

일반 결과의 이동 칸 수는 `걸 = 3칸` 예시 외에 도·개·걸·윷·모 전체 표가 정본에 없다. Issue #17에서 사용자 승인을 받아 확정하기 전까지 전진 플래너는 양의 칸 수를 직접 입력받으며, `YutResult`를 칸 수로 변환하는 연결 코드는 구현하지 않는다.

최소 WebSocket 명령·이벤트와 재접속 스냅샷 스키마는 초안이 마련되었다. 시작 확인, 멱등 처리, 일시 정지 및 저장 장애 정책은 확정되었다. 실제 네트워크 제품 구현 전에 아래 나머지 프로토콜 세부 정책을 결정해야 한다.

## 프로토콜 세부 정책 미결정

- 경기 전 방 이벤트와 채팅에 사용할 sequence 범위
- sequence 공백 감지 시 재동기화 요청과 오류 코드
- 최근 채팅 스냅샷의 정확한 개수와 페이지 경계
- `ERROR` 이벤트의 표준 오류 코드 목록

## 방·운영 흐름 미결정

- 팀 또는 방 설정 변경 시 준비 상태를 초기화할 정확한 대상
- 운영자 강제 종료 시 전적과 무효 사유

## 구현 전 ADR 필요

- raylib 클라이언트 언어: C 또는 C++
- Go HTTP/WebSocket 프레임워크
- Google 로그인 셸과 WASM 연동 방식
- 이벤트 저장 방식과 인덱스
- 방 actor/goroutine 모델

## 제품 세부 정책 미결정

- 닉네임 최소/최대 길이
- 닉네임 허용 문자
- 방 제목 유무 및 길이
- 방 비밀번호 길이·시도 제한
- 채팅·게임 로그 보존 기간
- 사용자 계정 삭제 정책
- 브라우저 최소 지원 버전
- 관리자 역할과 인증 방식

## UI 조정 가능 항목

- `spec/board_graph.yaml`의 렌더링 좌표는 참고 그림에서 얻은 근사값이다.
- 화면 비율, 모바일 배치, 말 겹침 오프셋은 실제 클라이언트 구현 중 조정한다.
- 좌표 조정은 논리 그래프의 노드와 연결을 바꾸지 않는다.

## 비차단 권장값

- 닉네임 2~20 grapheme
- 방 제목 1~50 grapheme
- 방 비밀번호 최대 64자
- 최근 채팅 재접속 복구 100개
- 프로토콜 JSON
- v1 DB SQLite. 단일 서버·로컬 디스크 전제가 깨지거나 부하 시험에서 전환 조건이 확인되면 PostgreSQL 재검토
