...
| 2 | 메인 로비 | `GET /api/v1/rooms` + `lobby` 전체 채팅 | 공통 위젯 세트(패널·버튼·목록 행), 방 상태 아이콘 |
...
### Issue #166 임시 비주얼 시스템

배포 전 사용자가 최종 아트워크를 확정할 때까지 `Pine & Paper` 스타일을 사용한다.
한지색 `#fffcf5`, 소나무색 `#245a46`, 먹색 `#243e35`, 주홍색 `#ba4837`을 기본으로
하고 팀 A/B의 기존 빨강/파랑과 형태 구분을 유지한다. 화면별 와이어프레임과 서버
후보 기반 입력은 유지한다. 로그인 타이틀·윷 장식은 CSS, 논리 말판은 C 드로잉,
기존 말·윷 결과는 PNG 리소스를 사용한다. 장식은 실제 결과를 나타내지 않으며 접근성
트리에서 제외한다. 공통 여백·모서리·폼·팝업은 `client/web/screens.css`에서 교체한다.
일반 버튼은 최소 44px(말판 좌표 선택 영역은 기존 규격 유지), 키보드 포커스는
3px 외곽선으로 표시하고 동작 감소 설정을 존중한다.
로그인/로비/방/게임은 각각 독립 화면이며 기존 고정 패널 내 스크롤을 유지한다.
작은 화면에서는 말판 → 게임 조작 → 채팅 순으로 배치한다. 실제 Noto Sans KR
폰트와 OFL 고지를 포함하며, 폰트·이미지 변경도 WASM 재빌드 의존성에 포함한다.
최초 인증 사용자는 닉네임을 저장하기 전까지 방 목록·채팅·게임 WebSocket에
접근할 수 없다. 프로필 모달의 취소와 Esc도 잠기며, 저장이 성공한 뒤에만
인증된 로비 기능을 활성화한다. 이미 닉네임을 저장한 사용자는 이 절차를
통과해 기존 화면으로 진입한다.
